...
Panopto.Util.defaultDeliveryName = "default";
...
Panopto.Util.getDeliveryName = function (deliveryItem) {
    ...
    return (deliveryName && (deliveryName != Panopto.Util.defaultDeliveryName))
...
Panopto.Util.getDateMessage = function (sessionRow) {
    switch (sessionRow.Status) {
        ...
        case Panopto.Data.SessionStatus.Scheduled:
            return Panopto.GlobalResources.Sessions_List_ScheduledFor.format(Panopto.Util.displayDate(sessionRow.StartTime, { format: "short" }));
...
Panopto.Util.displayPresenterMessage = function (session) {
    ...
    if (session.PresenterFirstNames
        ...
        presenterName = session.PresenterFirstNames[0] + " " + session.PresenterLastNames[0];
...
// Used for the TranscriptionRequestList to display a boolean
// as a yes/no value.
Panopto.Util.displayBoolean = function (booleanValue) {
    return booleanValue
        ? Panopto.GlobalResources.Yes
        : Panopto.GlobalResources.No;
};
...
// Specific to delivery data modal for list pages. Includes the query for automatic context results viewer search.
Panopto.Util.getViewerLinkWithQuery = function (item, time) {
    var queryText = Panopto.Application.defaultInstance.getSearchText();
    queryText = encodeURIComponent(queryText);
    return item.ViewerUrl + (queryText ? "&query=" + queryText : "") + (time ? "&start=" + time : "");
};
// Specific to modals
...
Panopto.Util.handleThumbnailLoad = function ($container) {
    ...
    $container.find("a:visible .thumbnail-img, span:visible .thumbnail-img")
        .on("load", function (e) {
        loadThumbnail($(e.currentTarget), true);
    })
        // IE8 (and 9?) may have already fired the "load" event for cached images
        .each(function (index, element) {
        element.complete && loadThumbnail($(element), true);
    })
        .on("error", function (e) {
        loadThumbnail($(e.currentTarget), false);
    });
};
...
Panopto.Util.verticallyCenterThumbnail = function ($thumbnail, $thumbnailContainer) {
    if ($thumbnail.height() < $thumbnailContainer.height()) {
        var verticalPadding = ($thumbnailContainer.height() - $thumbnail.height()) / 2;
        $thumbnail.css("padding", verticalPadding + "px 0");
    }
};
...
Panopto.Util.handleResize = function (isList, hidePagination) {
    ...
    if (isList) {
        var paginationHeight = hidePagination ? 0 : $(".pagination").outerHeight();
        ...
        var padding = parseInt($(".content-page").css('padding-top'), 10) + parseInt($(".content-page").css('padding-bottom'), 10);
...
// Respond to clicks on navbar folders and clicks on folder name links on video thumbnails
Panopto.Util.navigateToFolderSessionList = function (folderId) {
    // if we're moving between session lists we can update state to navigate to the specified folder instead of doing a full page refresh
    if (Panopto.SessionList) {
    ...
    // if we are not in a session list already or are going to a department home we will need to do a full refresh
    else {
        var sessionsPath = Panopto.appRoot + Panopto.Core.Constants.SessionsListAppRelativeUrl;
        // Load sessions page with folder selected (or redirect to department home, if any).
        location.href = Panopto.Core.StringHelpers.addQueryParameter(sessionsPath, { folderID: folderId });
    }
};
...
//# sourceMappingURL=file://seasyn/tfsbuilds/panopto_core_hohno-7a0-revert-saml_x64/20191031.7/_PublishedWebsites/WebUI/Scripts/Panopto/Util.js.map
