...
Panopto.ModalPopup = function () {
        ...
        itemRendered: function (sender, args) {
            ...
            if ((data.HideOnMaster && Panopto.isHostedMaster)
                ...
                || ((data.RequiredRole === Panopto.Data.AclRoleType.Creator)
                    && (!_this.userHasCreatorRole && !Panopto.user.isVideographer))
                || (data.RequiresWriteAccess && !_this.userHasWriteAccess)
                || (data.AccessRoles && !_this.hasAccessRoles(data.AccessRoles))) {
                item.style.display = "none";
            }
            // shouldShow is an optional predicate the tab can have.
            // call() should show with this context.
            if (data.shouldShow && !data.shouldShow.call(_this)) {
                item.style.display = "none";
            }
            Panopto.Core.UI.Handlers.button($(item), function () {
                // Check if modal page specifies a pre-unload confirmation message.
                if (_this._checkLeavePageConfirmation()) {
    ...
    $("#modalIframe").on("load", function (e) {
        ...
        if (Panopto.ModalPopup.defaultInstance._page) {
            ...
            catch (e) { /* Avoid mixed content console errors on certain modals */ }
    ...
    Panopto.Application.defaultInstance.add_navigate(function (sender, args) { return _this._handleNavigate(sender, args); });
};
Panopto.ModalPopup.prototype =
    {
        ...
        animateIn: function () {
            ...
            modalWindow
                ...
                opacity: "1",
                top: "0px"
            }, this.fadeInterval, "easeOutQuad", function onComplete() {
                modalWindow.show();
                // If this is the modal for creating a new folder we want to focus on the name input field
                if (modalWindow.hasClass("create-folder-window")) {
                    var iframe = modalWindow.find("iframe")[0];
                    var iframeBody = iframe.contentDocument.body;
                    $(iframeBody)
                        .find("#ModalContentPlaceHolder_Name")
                        .focus();
                }
                Panopto.ModalPopup.defaultInstance.resizeContent();
            });
        ...
        animateOut: function () {
            ...
            $(".modal-background").fadeOut(this.fadeInterval, function onComplete() {
            ...
            $("#modalWindow")
                ...
                opacity: "0",
                top: "20px"
            }, this.fadeInterval, "easeInQuad", function onComplete() {
                $("#modalWindow").hide();
            });
        },
        updateFrame: function () {
            ...
            // There is no navigation item, only the modal title.
            else {
        ...
        hide: function (handler, clear) {
            if (handler && typeof (handler) === "function") {
                ...
                for (var _i = 0, _a = this.onHide; _i < _a.length; _i++) {
                    handler = _a[_i];
                    handler();
                }
        ...
        _setPage: function (page) {
            ...
            var url = PanoptoTS.StringHelpers.format("{0}://{1}{2}{3}", scheme, Panopto.webServerFQDN, this._pageSet.baseUrl, pageData.URL);
        ...
        _handleNavigate: function (sender, args) {
            ...
            if (this._checkLeavePageConfirmation()) {
                ...
                for (var _i = 0, _a = this.onNavigate; _i < _a.length; _i++) {
                    var handler = _a[_i];
                    handler();
                }
        ...
        _checkLeavePageConfirmation: function () {
            ...
            try {
                ...
                if (confirmationMessage) {
                    ...
                    if (confirm(confirmationMessage)) {
                        ...
                        if (iframeWindow.Panopto) {
                            iframeWindow.Panopto.leavePageConfirmationMessage = null;
                        }
                        else if (Panopto.leavePageConfirmationMessage) {
                            Panopto.leavePageConfirmationMessage = null;
                        }
            ...
            catch (e) { /* void mixed content errors on certain modal pages */ }
...
Panopto.ModalPopup.pageSets =
    {
        ...
        SessionLinks: {
            ...
            pages: [
                ...
                {
                    Name: "SessionShare",
                    URL: (Panopto.useFreeTrialTerminology && !Panopto.user.isAdmin)
                        ? "ProShare.aspx"
                        : "Share.aspx",
                    HideOnMaster: true,
                    RequiresWriteAccess: true,
                    HelpUrl: Panopto.useFreeTrialTerminology
                        ? Panopto.freeTrialSupportUrl
                        : "http://support.panopto.com/documentation/video-management/how-do-i-share-session"
                },
        ...
        BatchShareLinks: {
            ...
            pages: [
                {
                    Name: "BatchShare", URL: "Share.aspx",
                    HelpUrl: Panopto.useFreeTrialTerminology
                        ? Panopto.freeTrialSupportUrl
                        : "http://support.panopto.com/documentation/video-management/how-batch-share-folder-or-session"
                }
            ]
        },
        CreateFolderLinks: {
        ...
        FolderLinks: {
            ...
            pages: [
                ...
                {
                    Name: "FolderShare", URL: "Share.aspx", HideOnMaster: true,
                    HelpUrl: Panopto.useFreeTrialTerminology
                        ? Panopto.freeTrialSupportUrl
                        : "http://support.panopto.com/documentation/video-management/how-do-i-share-folder"
                },
        ...
        UserLinks: {
            ...
            pages: [
                ...
                { Name: "UserConnections", URL: "Connections.aspx",
                    shouldShow: function () {
                        return Panopto.features.restrictSharingToConnectedUsers;
                    }
                },
                { Name: "Billing", URL: "Billing.aspx",
                    shouldShow: function () {
                        return Panopto.features.recurlyEnabled;
                    }
                },
                ...
                { Name: "UserPreset", URL: "Preset.aspx", RequiredRole: Panopto.Data.AclRoleType.Admin },
                {
                    ...
                    shouldShow: function () {
                        // This menu should be visible if usersCanCreateOauthClients is not "none" and
                        //  the user is an admin or usersCanCreateOauthClients is "allusers"
                        var roleAllowed = (Panopto.features.usersCanCreateOauthClients !== "none"
                            && (Panopto.user.isAdmin || Panopto.features.usersCanCreateOauthClients === "allusers"));
                        return roleAllowed;
                    }
                }
            ]
        ...
        RemoteRecorderLinks: {
            ...
            pages: [
                ...
                { Name: "RemoteRecorderAccess", URL: "Access.aspx", AccessRoles: [Panopto.Core.AccessRoleType.EditAccessEntries] },
                {
                    ...
                    shouldShow: function () {
                        // only show this tab if the feature is enabled
        ...
        RemoteRecorderRegistrationLinks: {
            ...
            baseUrl: Panopto.appRoot + "/Pages/Admin/RemoteRecorders/",
            pages: [
                { Name: "RemoteRecorderRegistration", URL: "RecorderRegistration.aspx" }
            ]
        },
        CreatePlaylistLinks: {
...
Panopto.ModalPopup.isInSet = function (setName, pageName) {
    ...
    var matches = $.grep(Panopto.ModalPopup.pageSets[setName].pages, function (item) { return item.Name === pageName; });
    // Convert to bool.
...
//# sourceMappingURL=file://seasyn/tfsbuilds/panopto_core_hohno-7a0-revert-saml_x64/20191031.7/_PublishedWebsites/WebUI/Scripts/Panopto/Controls/ModalPopup.js.map
